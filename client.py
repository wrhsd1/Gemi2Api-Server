...
def _extract_inline_generated_entries(candidate: list[Any]) -> list[list[Any]]:
    """Handle new Gemini response format where generated images live under candidate[12][0]['8']."""

    inline_entries: list[list[Any]] = []

    if not isinstance(candidate, list) or len(candidate) <= 12:
        return inline_entries

    attachments = candidate[12]
        ...
        **kwargs,
        ...
        if response.status_code != 200:
            ...
            try:
                ...
                dump_path = os.getenv("GEMINI_DUMP_RAW")
                if dump_path:
                    try:
                        Path(dump_path).write_text(
                            json.dumps(response_json, option=json.OPT_INDENT_2).decode(),
                            encoding="utf-8",
                        )
                        logger.debug(f"Raw response dumped to {dump_path}")
                    except Exception as dump_error:
                        logger.warning(f"Failed to dump raw response to {dump_path}: {dump_error}")
            ...
            try:
                ...
                for candidate_index, candidate in enumerate(candidate_list):
                    ...
                    # Generated images
                    generated_images = []
                    legacy_generated_nodes = get_nested_value(candidate, [12, 7, 0])
                    if legacy_generated_nodes:
                        img_body = None
                        ...
                        inline_entries = _extract_inline_generated_entries(candidate)
                        if inline_entries:
                            text = re.sub(
                                r"http://googleusercontent\.com/image_generation_content/\d+",
                                "",
                        ...
                        for idx, entry in enumerate(inline_entries, start=1):
                            ...
                            title = meta[2] or f"[Generated Image {idx}]"
                            alt = ""
